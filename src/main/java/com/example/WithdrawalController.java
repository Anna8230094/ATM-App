    ...
    public void handleEnter(@SuppressWarnings("exports") ActionEvent event) throws IOException, SQLException {
       ...
       Users logginUser = GreetingController.logginUser;
        ...
        if(!textfield.getText().equals("") && checkBox.isSelected()){
            ...
            if(amount<=50 && amount<= logginUser.getBalance()){
    
                logginUser.withdrawal(amount);
                ds.modifyBalances(logginUser.getPin(), logginUser.getBalance());
                AtmClass.atmClass.withdrawal(amount);
                resultController.setResultMessage("Withdrawal successful! Amount: " + amount);
            
                ds.insertUser(logginUser,amount, "withdrawal", LocalDate.now());

            }else if(amount>logginUser.getBalance()){
                resultController.setResultMessage("Your bank balance is less than ammount!");
            }else{
                resultController.setResultMessage("The daily limit is 50$!");
            }
